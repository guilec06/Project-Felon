    ...
    class ComponentPool : public IComponentPool {
        public:
            ComponentPool() : m_cache_dirty(true) {
                // Reserve capacity but don't pre-fill
            ...
            T &addComponent(EntityID e) {
                ...
                m_data.dense_components.emplace_back();  // Default construct component
            ...
            T &getComponent(EntityID e) {
                ...
                return m_data.dense_components[m_data.sparse[e]].component;  // O(1) cache-friendly access!
            }

            const std::vector<EntityID> &getActiveEntities() {
                if (m_cache_dirty) {
                    // Just copy the dense entities - they're already packed!
                    m_cached_entities.clear();
                    m_cached_entities.reserve(m_data.dense_components.size());
                    for (const auto& dense_comp : m_data.dense_components) {
                        m_cached_entities.push_back(dense_comp.entity);
                    }
                    m_cache_dirty = false;
                }
        ...
        private:
            ...
            // REMOVED: Old map, free list, counters
            ...
            // REMOVED: denseGrow - dense arrays grow automatically
    };
}
